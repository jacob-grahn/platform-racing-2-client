...
prompt='Read README.md and TODO.md. Pick the top incomplete TODO item. Either implement it fully, or break the TODO item into subtasks and implement one subtask fully. Run the relevant tests. Update TODO.md. Stop after one item. If there are no actionable TODO items left, reply exactly DONE and make no changes. If you made a change, reply with only a short git commit message, 72 characters or fewer, with no bullet, code fence, or explanation.'
