...
DEFAULT_BITMAP_REPORT = os.path.join("art", "timeline-bitmap-report.json")
...
def main(argv=None):
    ...
    bitmap_fallbacks = None
    if os.path.exists(DEFAULT_BITMAP_REPORT):
        with open(DEFAULT_BITMAP_REPORT, "r", encoding="utf-8") as handle:
            bitmap_fallbacks = json.load(handle)["fallbacks"]
    metadata = apply_plan(metadata, build_plan(metadata), bitmap_fallbacks)
