...
def parse_svg_path(svg_root, path):
    ...
    parts = rel.parts
    if len(parts) < 4 or parts[0] != "character":
        return None
    kind = parts[1]
    part = parts[2]
    channel = path.stem
...
def discover_jobs(svg_root, kinds, channels, limit):
    jobs = []
...
def build_atlases(records, atlas_root, padding, max_size):
    ...
    for record in records:
        key = (record["kind"], record["channel"])
        groups.setdefault(key, []).append(record)

    atlas_records = []
    for (kind, channel), group in sorted(groups.items()):
        entries = []
        for record in sorted(group, key=lambda item: (item["id"] or 0, item["part"])):
        ...
        out_dir = atlas_root / "character" / kind
        ...
        for page_index, (placements, width, height) in enumerate(pages, start=1):
            ...
            image_path = out_dir / f"{channel}@{group[0]['scale']}x{page_suffix}.png"
            json_path = out_dir / f"{channel}@{group[0]['scale']}x{page_suffix}.json"
...
def main(argv=None):
    ...
    jobs = discover_jobs(args.svg_root, args.kind, args.channel, args.limit)
    if not jobs:
