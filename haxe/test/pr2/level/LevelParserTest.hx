...
class LevelParserTest {
	...
	private static function testFlatFixture():Void {
		var fixture = LevelParser.parse(File.getContent("test/fixtures/flat-level.json"));
		...
		assertBlock(fixture.blockAtAny(2, 9), Start, "start block", false);
		...
		assertBlock(LevelParser.parse(minimalFixture('"type":"crumble"')).blockAt(1, 2), Crumble, "crumble block");
		assertBlock(LevelParser.parse(minimalFixture('"type":"item","options":"4"')).blockAt(1, 2), Item, "item block");
		assertBlock(LevelParser.parse(minimalFixture('"type":"infinite_item","options":"4"')).blockAt(1, 2), InfiniteItem, "infinite item block");
		assertBlock(LevelParser.parse(minimalFixture('"type":"vanish"')).blockAt(1, 2), Vanish, "vanish block");
		var teleport = LevelParser.parse(minimalFixture('"type":"teleport","options":"255"')).blockAt(1, 2);
	...
	private static function testValidationRejectsBadBlockTypes():Void {
		assertThrows(function() {
			LevelParser.parse(minimalFixture('"type":"mystery"'));
		}, "unknown block types are rejected");
	}

	private static function testValidationRejectsOutOfBoundsPositions():Void {
		assertThrows(function() {
			LevelParser.parse(minimalFixture('"type":"basic","x":99'));
		}, "out-of-bounds blocks are rejected");
		assertThrows(function() {
			LevelParser.parse(minimalFixture('"type":"basic"', '"playerStart":{"x":99,"y":1},'));
