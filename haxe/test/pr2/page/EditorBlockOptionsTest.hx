...
class EditorBlockOptionsTest {
	...
	private static function testAuthoredSaveLevelView():Void {
		...
		assertNear(100, title.controlWidth, "save-level title input base width");
		assertNear(2.03001403808594, title.scaleX, "save-level title input scale x");
		...
		assertNear(100, note.controlWidth, "save-level note base width");
		assertNear(44, note.controlHeight, "save-level note base height");
		assertNear(2.030029296875, note.scaleX, "save-level note scale x");
		assertNear(1.3865966796875, note.scaleY, "save-level note scale y");
