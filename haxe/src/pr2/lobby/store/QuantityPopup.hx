...
class QuantityPopup extends Popup {
	...
	public function new(listing:StoreListingData, onBuy:Int->Void) {
		...
		var slider = Std.downcast(DisplayUtil.findByName(art, "quantitySlider"), FlSlider);
	...
	private function bind(name:String, fn:Void->Void):Void { var b = LobbyArt.bind(DisplayUtil.findByName(art, name), fn); if (b != null) bindings.push(b); }
	private function changed(_:Event):Void { var slider = Std.downcast(DisplayUtil.findByName(art, "quantitySlider"), FlSlider); if (slider != null) numSelected = Std.int(slider.value); update(); }
	private function update():Void {
		...
		var buy = Std.downcast(DisplayUtil.findByName(art, "buy_bt"), FlButton); if (buy != null) buy.enabled = totalCost <= StorePopup.userCoins;
	}
	...
	override public function remove():Void {
		...
		var slider = Std.downcast(DisplayUtil.findByName(art, "quantitySlider"), FlSlider); if (slider != null) slider.removeEventListener(Event.CHANGE, changed);
