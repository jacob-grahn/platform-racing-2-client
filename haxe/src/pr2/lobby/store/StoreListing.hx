...
class StoreListing extends Sprite {
	...
	public function new(listing:StoreListingData) {
		...
		cover = Std.downcast(LobbyArt.findByName(art, "cover"), Sprite);
		...
		if (listing.imageUrl != "") {
			var holder = Std.downcast(LobbyArt.findByName(art, "picHolder"), Sprite);
	...
	private function hover(e:MouseEvent):Void { var bg = LobbyArt.findByName(art, "bg"); if (bg != null) bg.visible = e.type == MouseEvent.MOUSE_OVER; }
