package com.hurlant.crypto.symmetric;

import haxe.io.Bytes;
import pr2.crypto.ByteArrayCompat;

class AESKey implements ISymmetricKey {
	private static final SBOX:Array<Int> = [
		,
	];
	private static final RCON:Array<Int> = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36];
	private static final INV_SBOX:Array<Int> = buildInvSbox();

	private var key:Null<ByteArrayCompat>;
	private var keyLength:Int;
	private var nr:Int;
	private var roundKeys:Array<Int>;

	public function new(key:ByteArrayCompat) {
		keyLength = key.length;
		if (keyLength != 16 && keyLength != 24 && keyLength != 32) {
			throw 'AES key must be 16, 24, or 32 bytes, got $keyLength';
		}
		this.key = key.clone();
		nr = Std.int(keyLength / 4) + 6;
		roundKeys = expandKey(this.key.toBytes());
	}

	public function getBlockSize():Int {
		return 16;
	}

	public function encrypt(block:ByteArrayCompat, index:Int = 0):Void {
		var out = encryptBlock(readBlock(block, index));
		writeBlock(block, index, out);
	}

	public function decrypt(block:ByteArrayCompat, index:Int = 0):Void {
		var out = decryptBlock(readBlock(block, index));
		writeBlock(block, index, out);
	}

	public function dispose():Void {
		if (key != null) {
			for (i in 0...key.length) {
				key.set(i, 0);
			}
			key.length = 0;
			key = null;
		}
		for (i in 0...roundKeys.length) {
			roundKeys[i] = 0;
		}
		roundKeys = [];
		keyLength = 0;
		nr = 0;
	}

	public function toString():String {
		return "aes" + (8 * keyLength);
	}

	private function readBlock(block:ByteArrayCompat, index:Int):Bytes {
		var out = Bytes.alloc(16);
		for (i in 0...16) {
			out.set(i, block.get(index + i));
		}
		return out;
	}

	private function writeBlock(block:ByteArrayCompat, index:Int, bytes:Bytes):Void {
		for (i in 0...16) {
			block.set(index + i, bytes.get(i));
		}
	}

	private function encryptBlock(input:Bytes):Bytes {
		var state = [for (i in 0...16) input.get(i)];
		addRoundKey(state, 0);
		for (round in 1...nr) {
			subBytes(state);
			shiftRows(state);
			mixColumns(state);
			addRoundKey(state, round);
		}
		subBytes(state);
		shiftRows(state);
		addRoundKey(state, nr);
		return stateToBytes(state);
	}

	private function decryptBlock(input:Bytes):Bytes {
		var state = [for (i in 0...16) input.get(i)];
		addRoundKey(state, nr);
		for (round in 0...(nr - 1)) {
			invShiftRows(state);
			invSubBytes(state);
			addRoundKey(state, nr - 1 - round);
			invMixColumns(state);
		}
		invShiftRows(state);
		invSubBytes(state);
		addRoundKey(state, 0);
		return stateToBytes(state);
	}

	private function addRoundKey(state:Array<Int>, round:Int):Void {
		var offset = round * 16;
		for (i in 0...16) {
			state[i] = (state[i] ^ roundKeys[offset + i]) & 0xFF;
		}
	}

	private static function stateToBytes(state:Array<Int>):Bytes {
		var out = Bytes.alloc(16);
		for (i in 0...16) {
			out.set(i, state[i] & 0xFF);
		}
		return out;
	}

	private static function subBytes(state:Array<Int>):Void {
		for (i in 0...16) {
			state[i] = SBOX[state[i]];
		}
	}

	private static function invSubBytes(state:Array<Int>):Void {
		for (i in 0...16) {
			state[i] = INV_SBOX[state[i]];
		}
	}

	private static function shiftRows(state:Array<Int>):Void {
		var copy = state.copy();
		state[1] = copy[5];
		state[5] = copy[9];
		state[9] = copy[13];
		state[13] = copy[1];
		state[2] = copy[10];
		state[6] = copy[14];
		state[10] = copy[2];
		state[14] = copy[6];
		state[3] = copy[15];
		state[7] = copy[3];
		state[11] = copy[7];
		state[15] = copy[11];
	}

	private static function invShiftRows(state:Array<Int>):Void {
		var copy = state.copy();
		state[1] = copy[13];
		state[5] = copy[1];
		state[9] = copy[5];
		state[13] = copy[9];
		state[2] = copy[10];
		state[6] = copy[14];
		state[10] = copy[2];
		state[14] = copy[6];
		state[3] = copy[7];
		state[7] = copy[11];
		state[11] = copy[15];
		state[15] = copy[3];
	}

	private static function mixColumns(state:Array<Int>):Void {
		for (c in 0...4) {
			var i = c * 4;
			var a0 = state[i];
			var a1 = state[i + 1];
			var a2 = state[i + 2];
			var a3 = state[i + 3];
			state[i] = mul2(a0) ^ mul3(a1) ^ a2 ^ a3;
			state[i + 1] = a0 ^ mul2(a1) ^ mul3(a2) ^ a3;
			state[i + 2] = a0 ^ a1 ^ mul2(a2) ^ mul3(a3);
			state[i + 3] = mul3(a0) ^ a1 ^ a2 ^ mul2(a3);
		}
	}

	private static function invMixColumns(state:Array<Int>):Void {
		for (c in 0...4) {
			var i = c * 4;
			var a0 = state[i];
			var a1 = state[i + 1];
			var a2 = state[i + 2];
			var a3 = state[i + 3];
			state[i] = mul(0x0e, a0) ^ mul(0x0b, a1) ^ mul(0x0d, a2) ^ mul(0x09, a3);
			state[i + 1] = mul(0x09, a0) ^ mul(0x0e, a1) ^ mul(0x0b, a2) ^ mul(0x0d, a3);
			state[i + 2] = mul(0x0d, a0) ^ mul(0x09, a1) ^ mul(0x0e, a2) ^ mul(0x0b, a3);
			state[i + 3] = mul(0x0b, a0) ^ mul(0x0d, a1) ^ mul(0x09, a2) ^ mul(0x0e, a3);
		}
	}

	private static function expandKey(key:Bytes):Array<Int> {
		var keyLength = key.length;
		var nk = Std.int(keyLength / 4);
		var nr = nk + 6;
		var totalBytes = 16 * (nr + 1);
		var expanded = [for (_ in 0...totalBytes) 0];
		for (i in 0...keyLength) {
			expanded[i] = key.get(i);
		}

		var bytesGenerated = keyLength;
		var rconIndex = 0;
		var temp = [0, 0, 0, 0];
		while (bytesGenerated < totalBytes) {
			for (i in 0...4) {
				temp[i] = expanded[bytesGenerated - 4 + i];
			}
			if (bytesGenerated % keyLength == 0) {
				var t = temp[0];
				temp[0] = SBOX[temp[1]] ^ RCON[rconIndex++];
				temp[1] = SBOX[temp[2]];
				temp[2] = SBOX[temp[3]];
				temp[3] = SBOX[t];
			} else if (keyLength == 32 && bytesGenerated % keyLength == 16) {
				for (i in 0...4) {
					temp[i] = SBOX[temp[i]];
				}
			}
			for (i in 0...4) {
				expanded[bytesGenerated] = (expanded[bytesGenerated - keyLength] ^ temp[i]) & 0xFF;
				bytesGenerated++;
			}
		}
		return expanded;
	}

	private static inline function mul2(value:Int):Int {
		var shifted = value << 1;
		return ((shifted & 0x100) != 0 ? shifted ^ 0x11b : shifted) & 0xFF;
	}

	private static inline function mul3(value:Int):Int {
		return mul2(value) ^ value;
	}

	private static function mul(a:Int, b:Int):Int {
		var result = 0;
		var aa = a;
		var bb = b;
		for (_ in 0...8) {
			if ((bb & 1) != 0) {
				result ^= aa;
			}
			var high = aa & 0x80;
			aa = (aa << 1) & 0xFF;
			if (high != 0) {
				aa ^= 0x1b;
			}
			bb >>= 1;
		}
		return result & 0xFF;
	}

	private static function buildInvSbox():Array<Int> {
		var inv = [for (_ in 0...256) 0];
		for (i in 0...SBOX.length) {
			inv[SBOX[i]] = i;
		}
		return inv;
	}
}
