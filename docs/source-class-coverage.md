...
| `flash/blocks/MineBlock.as` | `pr2.level.BlockType`, `pr2.harness.LocalPlayerController`, `pr2.level.ServerLevelRenderer` | ported | Launch/removal behavior, local activation emission, and explosion/block-piece visuals are covered by `LocalPlayerControllerTest` and `ServerLevelRendererTest`. |
...
| `flash/blocks/PushBlock.as` | `pr2.level.BlockType`, `pr2.harness.LocalPlayerController` | ported | Push-block movement, recursive destination push-block movement, and direction-payload local activation behavior are covered by `LocalPlayerControllerTest`. |
...
| `flash/blocks/SupplyBlock.as` | `pr2.level.BlockType`, `pr2.harness.LocalPlayerController` | ported | Shared one-use bump-only depletion behavior and grey transform visuals for finish/item/custom-stat/stat/life/time supplies are covered by `LocalPlayerControllerTest`. |
