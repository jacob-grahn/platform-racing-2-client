...
| AS3 source | Haxe/OpenFL target | Status | Notes |
| --- | --- | --- | --- |
| `flash/chat/ChatInstance.as` | `pr2.lobby.tabs.ChatTab`, `pr2.lobby.chat.ChatLog`, `pr2.lobby.chat.HtmlNameMaker`, `pr2.lobby.dialogs.ChatRoomInfoPopup` | ported | The authored chat tab, room persistence, `set_chat_room`, send routing, transcript rendering, lock-to-bottom behavior, Ctrl update toggle, and info-button hover popup lifecycle are represented in `ChatTab`/`ChatLog`; covered by `ChatTabTest` and `LobbyServicesTest`. |
| `flash/chat/ChatRoomInfoPopup.as` | `pr2.lobby.dialogs.ChatRoomInfoPopup` | ported | The authored popup graphic, loading hide, `get_chat_rooms\`` request, `setChatRoomList` command registration/teardown, and `_sans` wrapped room-name rendering are covered by `ChatTabTest`. |
| `flash/chat/DeleteMessageButton.as` | `pr2.lobby.dialogs.MessagesItem`, `pr2.lobby.dialogs.HoverDelayPopup` | ported | The authored delete button graphic is mounted inside the Flash delayed-hover wrapper with exact title/body copy, cleanup, and delete-confirm routing; covered by `MessagesItemTest`. |
...
| `flash/chat/MessagesItem.as` | `pr2.lobby.dialogs.MessagesItem`, `pr2.lobby.chat.ChatText`, `pr2.lobby.chat.HtmlNameMaker` | ported | Sender names, guild marker, filter-aware body formatting, low-group escaping, Flash-style user/url/level/guild/invite rich links, carriage-return line breaks, clickable message-body links, locale-style sent dates, sent-time cursor/tint/long-date hover copy, action-button delayed hover wrappers, reply quote truncation, and full report/delete confirmation routing/copy are covered by `MessagesItemTest`. |
| `flash/chat/ReplyMessageButton.as` | `pr2.lobby.dialogs.MessagesItem`, `pr2.lobby.dialogs.HoverDelayPopup` | ported | The authored reply button graphic is mounted inside the Flash delayed-hover wrapper with exact title/body copy, cleanup, and `SendMessagePopup` quote routing; covered by `MessagesItemTest`. |
| `flash/chat/ReportMessageButton.as` | `pr2.lobby.dialogs.MessagesItem`, `pr2.lobby.dialogs.HoverDelayPopup` | ported | The authored report button graphic is mounted inside the Flash delayed-hover wrapper with exact title/body copy, cleanup, and moderation report routing; covered by `MessagesItemTest`. |
