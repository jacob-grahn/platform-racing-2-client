...
| `flash/character/ParticleEmitter.as` | `pr2.character.ParticleEmitter`, `pr2.character.ArrowSparkleEmitter` | partial | Flash-style interval spawning, randomized character-relative positions, life countdown, and cleanup are represented for arrow sparkles; the default StarEffect, rainbow-star, and Djinn particle uses remain under the remaining particle-runtime TODO. |
